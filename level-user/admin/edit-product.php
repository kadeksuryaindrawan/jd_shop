<?php  
  ...
  if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])){
  	$id_product = $_GET['id'];
  	$query_check_product = mysqli_query($connection, "SELECT * FROM tb_products WHERE id_product = $id_product");
  	if(mysqli_num_rows($query_check_product) == 1){
  		$data_product = mysqli_fetch_assoc($query_check_product);
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <form action="./process/edit-product.process.php" method="POST" enctype="multipart/form-data">
                      	<input type="hidden" name="input_id_product" value="<?= $data_product['id_product'] ?>">
                      	<div class="form-group">
                      		<label>Product Name</label>
                      		<input type="text" name="input_product_name" class="form-control" value="<?= $data_product['product_name'] ?>">
                      	</div>
                        <div class="form-group">
                          <label>Category Name</label>
                          <select name="input_category_name" class="form-control">
                            <?php 

                              $query_category = mysqli_query($connection, "SELECT * FROM tb_categories");
                              if(mysqli_num_rows($query_category) > 0){
                                while($data_category = mysqli_fetch_assoc($query_category)) {
                                	if($data_product['id_category'] == $data_category['id_category']){
                                		$isSelected = "Selected";
                                	}
                                	else{
                                		$isSelected = "";
                                  ...
                                  ?>
                                    <option value="<?= $data_category['id_category'] ?>" <?= $isSelected ?> ><?= $data_category['category_name'] ?></option>
                                  <?php
                                }
                              }
                              else{
                      	...
                      	<div class="form-group">
                      		...
                      		<input type="number" name="input_product_price" class="form-control" value="<?= $data_product['product_price'] ?>">
                      	</div>

                        <div class="form-group">
                          <label>Stock</label>
                          <input type="number" name="input_product_stock" class="form-control" value="<?= $data_product['product_stock'] ?>">
                        </div>
                        <div class="row">
                        <div class="col-md-6">
                          <label>Product Image 1</label><br/>
                          <img src="../../images/product/<?=$data_product['product_image'] ?>" style= "width: 50px; height: 50px;">
                          <input type="file" name="input_product_image" class="form-control">
                        </div>
                        <div class="col-md-6">
                          <label>Product Image 2</label><br/>
                          <img src="../../images/product/<?=$data_product['product_image_2'] ?>" style= "width: 50px; height: 50px;">
                          <input type="file" name="input_product_image_2" class="form-control">
                        </div>
                        </div>
